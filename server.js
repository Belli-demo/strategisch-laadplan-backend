...
// GET /geo/laadpalen/:id — bestaande laadpalen via de officiële MOW-WFS
...
// Zelfde route en response-vorm als voorheen (Overpass), zodat de frontend
...
app.get('/geo/laadpalen/:id', async (req, res) => {
  ...
  try {
    ...
    // Enkelvoudige aanhalingstekens verdubbelen voor een veilige CQL-filter
    const veilig = gemeenteNaam.replace(/'/g, "''");
    const cql = encodeURIComponent(`gemeente='${veilig}'`);
    const url = `https://geoserver.gis.cloud.mow.vlaanderen.be/geoserver/beleid/wfs?SERVICE=WFS&version=2.0.0&request=GetFeature&typeName=laadpunten_public&outputFormat=application/json&CQL_FILTER=${cql}`;
    ...
    try {
      ...
      data = await r.json();
      console.log(`  Laadpalen ${id}: ${data.features?.length} via MOW-WFS`);
    } catch(e) {
      console.warn(`  MOW-WFS fout: ${e.message}`);
    }

    if (!data) return res.status(503).json({ error: 'MOW-WFS niet beschikbaar', elements: [] });

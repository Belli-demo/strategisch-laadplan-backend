...
// ══════════════════════════════════════════════════════════════════════
// POSTCODES PER NIS (Statbel/Bpost conversietabel, nomenclatuur 2019-2024)
// ══════════════════════════════════════════════════════════════════════
// In-memory Map: NIS-code (string) → array van postcodes (strings).
// Wordt bij server-start uit lokaal bestand geladen (postcodes-per-nis.json
// in geo-data/). Bron: Statbel XLSX "Conversion Postal code_Refnis code_va01012019.xlsx"
// via https://statbel.fgov.be/nl/over-statbel/methodologie/classificaties/geografie.
// 581 gemeenten, 1152 postcode-koppelingen, peildatum 01/01/2019 (nomenclatuur
// die matcht met de NIS-codes in nis-lookup.js).
const POSTCODES_PATH = path.join(__dirname, 'geo-data', 'postcodes-per-nis.json');
let postcodesPerNis = new Map();

function initPostcodesPerNis() {
  try {
    const inhoud = fs.readFileSync(POSTCODES_PATH, 'utf8');
    const data = JSON.parse(inhoud);
    postcodesPerNis = new Map(Object.entries(data));
    console.log(`  Postcodes per NIS geladen: ${postcodesPerNis.size} gemeenten`);
  } catch(e) {
    console.warn(`  Postcodes per NIS-bestand niet gevonden op ${POSTCODES_PATH}. Endpoint /geo/postcodes/:nis zal 404 geven tot dit bestand aanwezig is.`);
  }
}
initPostcodesPerNis();

// GET /geo/postcodes/:nis — postcodes voor één NIS-code (bijv. 23088 → [1800, 1804])
  ...
  if (!postcodes) {
    return res.status(404).json({
      error: postcodesPerNis.size === 0
        ? `Postcode-dataset niet geladen. Ontbreekt ${POSTCODES_PATH}?`
        : `Geen postcodes gevonden voor NIS ${nis}.`,
      dataset_geladen: postcodesPerNis.size > 0,
      dataset_aantal_gemeenten: postcodesPerNis.size,
    });
  }
  res.json({ nis, postcodes, aantal: postcodes.length, bron: 'Statbel/Bpost 01-01-2019 (581 gemeenten)' });
});
